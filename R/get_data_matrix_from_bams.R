...
#' Fetch data matrix from NOMe-seq BAM files for a set of regions
#'
#'
#'
...
#'   \item{nFragsAnalyzed}{number of fragments analysed after filtering non-unique and bisulfite conversion failed fragments.}
...
#'   \item{nFragsClipped}{number of fragments clipped from both ends to get rid of partial footprints at the ends of fragments.}
#'   \item{DataMatrix}{\code{matrix} with corresponding data where columns reflect positions from start until end in \code{regions} and rows correspond to fragments.

#'   }
#' }
#' @export
#'
get_data_matrix_from_bams <- function(bamfiles,
																			samplenames,
																			regions,
																			genome,
	...
	message(paste0("Fetching data matrices from BAM files generated by ",alignerUsed,". Assumed enzyme ",SMFenzymeUsed,"."))
																			...
																			function(bamgroupname){
																				ctbl <- do.call(rbind,parallel::mclapply(seq_along(regions),
																																			 function(regi){
																																			 	...
																																			 	outlist <- fetch_data_matrix_from_bams_cpp(whichContext = whichContext,
																																			 	...
																																			 	outtbl <- tibble::tibble_row("SampleName" = bamgroupname,
																																			 															 ...
																																			 															 "DataMatrix" = list(data_mat))
																																			 	colnames(outtbl)[which(colnames(outtbl) == "DataMatrix")] <- paste0(whichContext,"_DataMatrix")
																																			 	return(outtbl)
																																			 },mc.cores = ncores))
